  ...
  return (
    <div className="px-6 md:px-10 py-6">
      ...
      <div className="mt-6 rounded-xl border border-neutral-200 bg-white p-4 md:p-6">
        <Input label="Judul" value={form.title} onChange={update('title')} placeholder="Tuliskan Nama kegiatan" className="mb-4" />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Cover Upload */}
          <div>
            <label className="mb-1 block text-sm font-medium text-neutral-700">Cover</label>
            <label className="flex items-center justify-between gap-3 rounded-lg border border-neutral-200 bg-white px-3 py-2.5 text-sm cursor-pointer">
              <span className="opacity-70 truncate">{form.cover?.name || 'Unggah file'}</span>
              <input type="file" className="hidden" onChange={(e) => setForm((s) => ({ ...s, cover: e.target.files?.[0] || null }))} accept="image/*" />
            </label>
          </div>

          <Input label="Tanggal Publikasi" type="date" value={form.publishDate} onChange={update('publishDate')} placeholder="dd/mm/yyyy" />
        </div>

        <Textarea className="mt-4" label="Deskripsi" value={form.description} onChange={update('description')} placeholder="Tuliskan Deskripsi kegiatan" />
